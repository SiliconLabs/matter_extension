...
err_t MQTT_Transport_SSLConfigure(MQTT_Transport_t *transP,
                                  const u8_t *ca,
                                  size_t ca_len,
                                  const u8_t *privkey,
                                  size_t privkey_len,
...
static int8_t transport_recv_from_nw(void *conn, void *buf, uint16_t len, uint16_t offset)
{
  ...
  if (!client->pcbRxStart)
    return 0;
  if (client->pcbRxStart->tot_len < (offset + len))
    return 0;
...
static err_t transport_connect_cb(void *arg, struct altcp_pcb *tpcb, err_t err)
{
  MQTT_Transport_t *client = (MQTT_Transport_t *)arg;
  if (err != ERR_OK) {
