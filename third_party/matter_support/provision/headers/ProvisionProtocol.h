...
public:
  ...
  virtual bool Execute(Storage *store, ByteSpan &request,
                       MutableByteSpan &response) = 0;
};
...
private:
  ...
  CHIP_ERROR GenerateCSR(Storage *store, Encoding::Buffer &in,
                         Encoding::Buffer &out);
  CHIP_ERROR Import(Storage *store, Encoding::Buffer &in,
                    Encoding::Buffer &out);
