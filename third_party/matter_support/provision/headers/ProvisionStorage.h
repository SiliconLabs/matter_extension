...
struct CustomStorage : public GenericStorage {
  ...
  CHIP_ERROR Get(uint16_t id, uint8_t *value, size_t max_size,
...
struct Storage
  ...
  static constexpr size_t kSpake2pVerifierB64LengthMax =
      BASE64_ENCODED_LEN(chip::Crypto::kSpake2p_VerifierSerialized_Length) + 1;
  static constexpr size_t kSpake2pSaltB64LengthMax =
  ...
  static constexpr size_t kTotalPayloadDataSizeInBits =
      (kVersionFieldLengthInBits + kVendorIDFieldLengthInBits +
       ...
       kPayloadDiscriminatorFieldLengthInBits + kSetupPINCodeFieldLengthInBits +
       kPaddingFieldLengthInBits);
  static constexpr size_t kTotalPayloadDataSize =
      kTotalPayloadDataSizeInBits / 8;
...
public:
  ...
  CHIP_ERROR GetDeviceAttestationCSR(uint16_t vid, uint16_t pid,
                                     const CharSpan &cn, MutableCharSpan &csr);
  ...
  void SetBufferSize(size_t size) {
    mBufferSize = size > 0 ? size : kArgumentSizeMax;
  }
