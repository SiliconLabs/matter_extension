...
def main():
    parser = argparse.ArgumentParser(
        description="Compare two code-size CSVs (current vs baseline) for a single toolchain."
    )
    parser.add_argument('csv1', help="First CSV (must contain 'current' or 'target' in its name).")
    parser.add_argument('csv2', help="Second CSV (must contain 'current' or 'target' in its name).")
    parser.add_argument(
        '--toolchain',
        default='',
        help="Label printed in the report header (e.g. 'gcc' or 'llvm'). Optional.",
    )
    args = parser.parse_args()

    csv1, csv2 = args.csv1, args.csv2
    toolchain_label = args.toolchain.strip().lower()
    ...
    tc_suffix = f" [toolchain: {toolchain_label}]" if toolchain_label else ""
    print(
        f"Comparing current build ({Path(current_file).name}) "
        f"against baseline ({Path(baseline_file).name}){tc_suffix}"
    )
    print(f"Thresholds: Flash > {FLASH_THRESHOLD_PCT}%, RAM > {RAM_THRESHOLD_PCT}%")
    print("=" * 100)

    tc_col = toolchain_label if toolchain_label else "-"
    print(f"{'Board':<15} {'Toolchain':<10} {'Flash Change':<25} {'RAM Change':<25} {'Status':<10}")
    print("-" * 100)
    ...
    for board, current in sorted(current_data.items()):
        if board in baseline_data:
            ...
            print(f"{board:<15} {tc_col:<10} {flash_str:<25} {ram_str:<25} {status:<10}")
        else:
            print(f"{board:<15} {tc_col:<10} {'NOT IN BASELINE':<25} {'NOT IN BASELINE':<25} {'WARN':<10}")

    print("=" * 100)

    if threshold_exceeded:
        print(f"Code size increase threshold exceeded{tc_suffix}!")
        sys.exit(1)
    else:
        print(f"All boards within acceptable code size thresholds{tc_suffix}")
        sys.exit(0)
