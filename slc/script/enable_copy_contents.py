...
SKIP_LIST = ['CHIP_PROJECT_CONFIG_INCLUDE', 'SYSTEM_PROJECT_CONFIG_INCLUDE',
             'INET_PROJECT_CONFIG_INCLUDE', 'BLE_PROJECT_CONFIG_INCLUDE']
NON_EXISTENT_FILES = []
RESOLVED_FILES = []
efr32_headers = ['PigweedLogger.h', 'Rpc.h', 'LEDWidget.h', 'matter_config.h',
                 'OTAConfig.h', 'BaseApplication.h', 'init_efrPlatform.h', 'efr32_utils.h']
std_c_headers = ["assert", "array", "new", "ctype", "stdbool.h", "errno", "float", "limits", "locale",
                 "math", "setjmp", "signal", "stdarg", "stddef", "stdint", "stdio", "stdlib", "string", "time", "stddef"]
std_cpp_headers = ["cassert", "cctype", "cerrno", "cfenv", "cfloat", "cinttypes", "ciso646", "climits", "clocale", "cmath", "csetjmp", "csignal",
                   "cstdarg", "cstdbool", "cstddef", "cstdint", "cstdio", "cstdlib", "cstring", "ctgmath", "ctime", "cuchar", "cwchar", "cwctype", "vector", "algorithm"]
found = False
root_inc_list = ['src', 'src/include', 'slc/inc', 'src/lib/support/pw_log_chip/public_overrides',
                 'src/lib/support/pw_log_chip/public', 'src/platform/silabs/efr32']
matter_component = {}
headers_found = 0
    ...
    if os.path.exists(header_abs_path):
        ...
        if idx is not None:
            ...
            if header in header_map[idx][include_path]:
                ...
                header_list[indx][include_path].append(header) if indx is not None else header_list.append({
                    str(include_path): [header]})
                return True
        else:
        ...
        if (include_reg_type1.match(line) is not None or ('#include' in line and '*' not in line)):
            ...
            if inc_line is None:
                ...
                if include_reg_type2.match(line) is not None and len(header_relpath.split('/')) < 2:
                    header_relPath = str(os.path.join(
                        str(os.path.dirname(file.replace(str(ROOT), ''))).lstrip('/'), header_relpath))
...
def update(inc_dir, val, file):
    ...
    for x in range(0, len(full_path.split('/')) - 1):
        ...
        idx = next((index for (index, d) in enumerate(
            matter_component['include']) if include_dir_val == d['path'].rstrip('/')), None)
        if idx is not None:
