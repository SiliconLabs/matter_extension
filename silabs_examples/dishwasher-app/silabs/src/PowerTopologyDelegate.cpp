...
CHIP_ERROR PowerTopologyDelegate::AddActiveEndpoint(EndpointId endpointId)
{    
    bool AvailableEndpointFlag = false;
    bool ActiveEndpointFlag = false;
    size_t availableIndexToAdd = MAX_ENDPOINT_COUNT;
    ...
    VerifyOrReturnError(AvailableEndpointFlag && !ActiveEndpointFlag && (availableIndexToAdd != MAX_ENDPOINT_COUNT), CHIP_ERROR_INVALID_ARGUMENT);
...
CHIP_ERROR PowerTopologyDelegate::RemoveActiveEndpoint(EndpointId endpointId)
{    
    for (uint16_t index = 0; index < MAX_ENDPOINT_COUNT; index++)
    {
        if (mActiveEndpointIdList[index] == endpointId)
...
void PowerTopologyDelegate::InitAvailableEndpointList()
{
    for (uint16_t index = 0; index < MAX_ENDPOINT_COUNT; index++)
    {
        mAvailableEndpointIdList[index] = (emberAfEndpointIndexIsEnabled(index))? 
                                           emberAfEndpointFromIndex(index) : kInvalidEndpointId;
...
{
    GetDelegate()->InitAvailableEndpointList();
